...
final class ApiErrorResponse<T> {
    ...
    static final class Serializer<T> extends StoneSerializer<ApiErrorResponse<T>> {
        ...
        @Override
        public void serialize(ApiErrorResponse<T> value, JsonGenerator g) throws IOException {
            throw new UnsupportedOperationException("Error wrapper serialization not supported.");
        }

        @Override
        public ApiErrorResponse<T> deserialize(JsonParser p) throws IOException {
            T error = null;
